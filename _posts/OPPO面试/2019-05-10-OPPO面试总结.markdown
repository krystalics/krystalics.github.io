...
- 静态变量：存放在方法区中的常量池中，如Student.class中的，birthday=ref
...
**线程池总结**：线程池的本质是一个HashSet，多余的任务会放在阻塞队列中。只有当阻塞队列满了之后，才会触发非核心线程的创建。所以非核心线程只是临时打杂的。还提供了两个钩子(beforeExecute,afterExecute)给我们，让我们可以在执行任务前后做一些处理。
...
**Full GC(也叫Major GC)是发生在老年代**的垃圾收集动作，采用**标记-清除算法**
...
**标记-压缩算法**：与算法标记-清除算法类似，都是先对存活的对象进行标记。但是**清除之后会把活的对象向左端空闲空间移动，然后再更新引用它们的指针**。弥补了标记-清除算法的缺点，但是成本也增加了，适用于老年代(旧生代)

<img src="https://github.com/krystalics/krystalics.github.io/blob/master/_posts/img/28.png?raw=true">

**标记-整理算法：**也是利用GC树先将所有存活对象标记，但事后续并不是清除，而是直接将存活对象向一端移动，然后再清除掉另一端的内存。和**标记-压缩**相差不多，执行顺序不一样。
